...
def house_card(points: int, ltc: float, usd: float) -> bytes:
    """Casino-style house bankroll card showing pts / LTC / USD."""
    W, H = 900, 420
    img = vgradient((W, H), BG_TOP, BG_BOTTOM).convert("RGBA")
    d = ImageDraw.Draw(img)

    # header bar
    rounded_rect(d, (24, 18, W - 24, 88), 18,
                 fill=(PANEL_TOP[0], PANEL_TOP[1], PANEL_TOP[2], 230),
                 outline=GOLD_DARK, width=2)
    neon_text(img, (44, 28), "🏦  HOUSE BANKROLL", font(32, True),
              fill=GOLD, glow=GOLD, glow_alpha=160, blur=8)

    # corner chip decorations
    for i, color in enumerate([GOLD, GREEN, NEON_BLUE, RED]):
        _draw_chip(img, (W - 80 - i * 58, 54), 20, color, WHITE)

    # central panel
    panel = felt_panel((W - 80, 250))
    img.alpha_composite(panel, dest=(40, 108))

    # ── Points row ──────────────────────────────────────────────────────────
    d.text((72, 128), "POINTS", font=font(18, False), fill=MUTED)
    pts_str = f"{points:,}"
    neon_text(img, (72, 150), pts_str, font(72, True),
              fill=GOLD, glow=GOLD, glow_alpha=200, blur=10)

    # divider
    d.line((72, 240, W - 72, 240), fill=(*GOLD_DARK, 120), width=1)

    # ── LTC / USD row ────────────────────────────────────────────────────────
    ltc_str = f"{ltc:.4f} LTC"
    usd_str = f"${usd:,.2f} USD"

    # LTC block (left half)
    mid = W // 2
    d.text((80, 256), "LTC VALUE", font=font(15, False), fill=MUTED)
    neon_text(img, (80, 278), ltc_str, font(34, True),
              fill=NEON_BLUE, glow=NEON_BLUE, glow_alpha=160, blur=6)

    # vertical divider
    d.line((mid, 250, mid, 340), fill=(*GOLD_DARK, 100), width=1)

    # USD block (right half)
    d.text((mid + 24, 256), "USD VALUE", font=font(15, False), fill=MUTED)
    neon_text(img, (mid + 24, 278), usd_str, font(34, True),
              fill=GREEN, glow=GREEN, glow_alpha=160, blur=6)

    # bottom label
    rate_str = "1 Point = 0.0001 LTC"
    tw = d.textlength(rate_str, font=font(15, False))
    d.text(((W - tw) / 2, 370), rate_str, font=font(15, False), fill=MUTED)
...
_ANIM_W, _ANIM_H = 600, 346   # all animation frames are downscaled to this size
_ANIM_DUR       = 130          # ms per frame — feels deliberate, not frantic
_ANIM_HOLD      = 3            # hold frames on final result before bot edits to static
...
def coinflip_anim_card(result: str, pick: str, bet: int, payout: int) -> bytes:
    ...
    for i in range(total_frames):
        ...
        d.text((CARD_W // 2 - 72, 410), f"Flipping... {pick.upper()}",
               font=font(22, True), fill=WHITE)
        frames.append(
            img.resize((_ANIM_W, _ANIM_H), Image.LANCZOS)
               .convert("P", palette=Image.Palette.ADAPTIVE, colors=128)
        )

    final_img = (
        Image.open(io.BytesIO(coinflip_card(result, pick, bet, payout)))
        .resize((_ANIM_W, _ANIM_H), Image.LANCZOS)
        .convert("P", palette=Image.Palette.ADAPTIVE, colors=128)
    )
    frames.extend([final_img] * _ANIM_HOLD)

    buf = io.BytesIO()
    frames[0].save(buf, format="GIF", save_all=True, append_images=frames[1:],
                   duration=_ANIM_DUR, loop=0, optimize=True, disposal=2)
    return buf.getvalue()


def slots_anim_card(reels: Sequence[str], bet: int, payout: int,
                    username: str = "", match_label: str = "") -> bytes:
    """Animated slot reels GIF — each reel stops one by one, then shows final card."""
    import random as _rng

    frames: List[Image.Image] = []
    TOTAL = 12
    STOP_AT = [4, 8, 11]   # frame index when reel 0, 1, 2 lock in
    final_syms = [_slot_kind(str(r)) for r in reels]
    ...
    for i in range(TOTAL):
        ...
        for ri in range(3):
            sx = start_x + ri * (slot_w + gap)
            locked = i >= STOP_AT[ri]
            rim = GOLD if locked else (180, 200, 255)
            rounded_rect(d, (sx, y, sx + slot_w, y + slot_h), 18,
                         fill=(18, 22, 32), outline=rim, width=3)
            rounded_rect(d, (sx + 6, y + 6, sx + slot_w - 6, y + slot_h - 6), 14,
                         outline=GOLD_DARK, width=1)
            sym = final_syms[ri] if locked else _rng.choice(SLOT_SYMBOLS)
            _draw_slot_symbol(img, (sx + slot_w // 2, y + slot_h // 2), 110, sym)

        if i < STOP_AT[2]:
            spin_f = font(22, True)
            tw = d.textlength("Spinning…", font=spin_f)
            d.text(((CARD_W - tw) / 2, CARD_H - 52), "Spinning\u2026",
                   font=spin_f, fill=MUTED)
    ...
    final_img = (
        Image.open(io.BytesIO(slots_card(reels, bet, payout, username, match_label)))
        .resize((_ANIM_W, _ANIM_H), Image.LANCZOS)
        .convert("P", palette=Image.Palette.ADAPTIVE, colors=128)
    )
    frames.extend([final_img] * _ANIM_HOLD)
...
def dice_anim_card(rolls: Sequence[int], bet: int, payout: int,
                   pick: Optional[int] = None) -> bytes:
    """Animated dice roll GIF — die tumbles with random pips then settles."""
    import random as _rng

    frames: List[Image.Image] = []
    TOTAL = 10
    SETTLE = 7   # frame index when die shows final value

    final_pip = int(rolls[0]) if rolls else 1

    for i in range(TOTAL):
        img, d = base_canvas("\u2684 DICE",
                             f"Bet {bet:,}" + (f"  \u2014  guessed {pick}" if pick else ""))
        panel = felt_panel((CARD_W - 80, 320))
        img.alpha_composite(panel, dest=(40, 130))

        pips = final_pip if i >= SETTLE else _rng.randint(1, 6)
        size = 200
        draw_die(img, ((CARD_W - size) // 2, 195), size, pips)

        if i < SETTLE:
            rf = font(24, True)
            tw = d.textlength("Rolling\u2026", font=rf)
            d.text(((CARD_W - tw) / 2, CARD_H - 58), "Rolling\u2026",
                   font=rf, fill=MUTED)
    ...
    final_img = (
        Image.open(io.BytesIO(dice_card(rolls, bet, payout, pick=pick)))
        .resize((_ANIM_W, _ANIM_H), Image.LANCZOS)
        .convert("P", palette=Image.Palette.ADAPTIVE, colors=128)
    )
    frames.extend([final_img] * _ANIM_HOLD)
...
def rps_anim_card(player: str, bot_move: str, bet: int, payout: int,
                  outcome: str = "") -> bytes:
    """Animated RPS GIF — both hands cycle then lock in one by one."""
    import random as _rng

    frames: List[Image.Image] = []
    TOTAL = 10
    PLAYER_LOCK = 5   # frame player hand freezes
    BOT_LOCK    = 8   # frame bot hand freezes

    glyphs = {"rock": "\u270A", "paper": "\u270B", "scissors": "\u270C"}
    all_moves = list(glyphs.keys())

    pg_final = glyphs.get(str(player).lower(), "\u270A")
    bg_final = glyphs.get(str(bot_move).lower(), "\u270A")

    for i in range(TOTAL):
        img, d = base_canvas("ROCK \u00b7 PAPER \u00b7 SCISSORS", f"Bet {bet:,}")
        panel = felt_panel((CARD_W - 80, 340))
        img.alpha_composite(panel, dest=(40, 130))

        pg = pg_final if i >= PLAYER_LOCK else glyphs[_rng.choice(all_moves)]
        bg = bg_final if i >= BOT_LOCK    else glyphs[_rng.choice(all_moves)]

        bigf = font(150, True)
        p_col = GOLD if i >= PLAYER_LOCK else WHITE
        b_col = GOLD if i >= BOT_LOCK    else WHITE
        d.text((110, 195), pg, font=bigf, fill=p_col)
        d.text((CARD_W - 290, 195), bg, font=bigf, fill=b_col)
        d.text((CARD_W // 2 - 26, 255), "VS", font=font(48, True), fill=GOLD)

        d.text((120, 385), "YOU", font=font(22, True), fill=MUTED)
        d.text((CARD_W - 240, 385), "BOT", font=font(22, True), fill=MUTED)

        if i < BOT_LOCK:
            sf = font(26, True)
            tw = d.textlength("Shoot!", font=sf)
            d.text(((CARD_W - tw) / 2, CARD_H - 60), "Shoot!",
                   font=sf, fill=GOLD)
    ...
    final_img = (
        Image.open(io.BytesIO(rps_card(player, bot_move, bet, payout, outcome=outcome)))
        .resize((_ANIM_W, _ANIM_H), Image.LANCZOS)
        .convert("P", palette=Image.Palette.ADAPTIVE, colors=128)
    )
    frames.extend([final_img] * _ANIM_HOLD)
...
def valentines_anim_card(username: str, reels: Sequence[str],
                          bet: int, payout: int) -> bytes:
    """Animated Valentines slot GIF — heart reels spin and stop one by one."""
    import random as _rng

    VAL_SYMS = ["\U0001f498", "\U0001f496", "\U0001f49d",
                "\U0001f339", "\U0001f36b", "\U0001f48d"]   # 💘💖💝🌹🍫💍
    PINK_RIM   = (220,  80, 130)
    PINK_DARK  = (140,  30,  70)
    PINK_BG    = ( 55,  10,  30)
    ...
    TOTAL  = 12
    STOP_AT = [4, 8, 11]
    final_syms = list(reels) if reels else [VAL_SYMS[0]] * 3

    for i in range(TOTAL):
        img, d = base_canvas("\u2665 VALENTINES", f"@{username}  \u00b7  Bet {bet:,}")
        panel = felt_panel((CARD_W - 80, 320))
        img.alpha_composite(panel, dest=(40, 130))
        ...
        for ri in range(3):
            sx = start_x + ri * (slot_w + gap)
            locked = i >= STOP_AT[ri]
            rim = (255, 120, 180) if locked else PINK_RIM
            rounded_rect(d, (sx, y, sx + slot_w, y + slot_h), 18,
                         fill=PINK_BG, outline=rim, width=3)
            rounded_rect(d, (sx + 6, y + 6, sx + slot_w - 6, y + slot_h - 6), 14,
                         outline=PINK_DARK, width=1)

            sym = final_syms[ri] if locked else _rng.choice(VAL_SYMS)
            sym_f = font(80, True)
            tw = d.textlength(sym, font=sym_f)
            d.text((sx + (slot_w - tw) / 2, y + slot_h // 2 - 52),
                   sym, font=sym_f, fill=WHITE)

        if i < STOP_AT[2]:
            sf = font(22, True)
            tw = d.textlength("Spinning with love\u2026", font=sf)
            d.text(((CARD_W - tw) / 2, CARD_H - 52),
                   "Spinning with love\u2026", font=sf, fill=(220, 120, 160))
    ...
    final_img = (
        Image.open(io.BytesIO(valentines_card(username, "", 100 if payout > 0 else 0, payout)))
        .resize((_ANIM_W, _ANIM_H), Image.LANCZOS)
        .convert("P", palette=Image.Palette.ADAPTIVE, colors=128)
    )
    frames.extend([final_img] * _ANIM_HOLD)
